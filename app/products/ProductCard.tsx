...
export default function ProductCard({ post, index }: { post: Tpost; index: number }) {
  ...
  return (
    ...
    >
      <div className="relative flex items-center justify-center p-3 overflow-hidden shadow-lg">
        ...
        >
          <Heart size={14} className={isFavorite(post.id) ? "fill-current" : ""} />
        </button>
      </div>

      <div className="p-3 flex flex-col flex-1 gap-1.5">
        <span className="w-6 h-0.5 bg-indigo-500 rounded-full" />
        <p className="text-slate-600 text-xs line-clamp-2 leading-relaxed flex-1">
          {post.description}
        </p>
        {post.price && (
        ...
        <div className="flex gap-4 mt-1">
          <Link
            ...
            className="flex-1 px-3 py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-lg text-center hover:bg-indigo-700 hover:-translate-y-0.5 active:scale-95 transition-all duration-200"
          >
            Details
          </Link>
          <button
            onClick={handleAddToCart}
            className={`px-3 py-2.5 rounded-lg text-xs font-bold transition-all duration-200 active:scale-95 ${
              isInCart(post.id)
